...
print("Read the data")
lines = [] #simple array to append all the entries present in the .csv file

with open('C:\\temp\\data\\data\\driving_log.csv') as csvfile: #currently after extracting the file is present in this path
    reader = csv.reader(csvfile)
    next(reader, None) #this is necessary to skip the first record as it contails ns the headings
    for line in reader:
        lines.append(line)
...
# 15 indicates 15% of the dataset is validation set
...
def generator(lines, batch_size=32):
    ...
    while 1: 
        shuffle(lines) #shuffling the total images
        for offset in range(0, num_samples, batch_size):
            ...
            for batch_sample in batch_samples:
                    for i in range(0,3): #we are taking 3 images, first one is center, second is left and third is right
                        ...
                        name = 'C:\\temp\\data\\data\\IMG\\'+batch_sample[i].split('\\')[-1]
                        ...
                        # We take the image and just flip it and invert (-1) the measurement
            ...
            # instead we yield the values which means we hold until the generator is running
...
# trim image: crop skye (70 pixel))and body (25 pixel) of car
...
# Using 25% dropout rate after first fully connected layer
...
# using mean squared error ("mse") loss function is the right choice for this regression problem
